...
if (( EMPTY_WORKING_DIR != 0 )); then
  rm -rf $DATA_ROOT
fi

mkdir -p $DATA_ROOT 2>/dev/null

if (( SKIP_C_PREPROCESS == 0 )); then
  ...
  echo SKIP_DECLS: ${SKIP_DECLS} 
  echo MAX_LEAVES: ${MAX_LEAVES} 
...
# NOTE(Sam): enable if using less than 200 training files
# train=2
# test=2
